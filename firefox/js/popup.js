...
browser.storage.onChanged.addListener(function({ghat,ghUserData,chainLength},area){
  if(area != "sync") return
  if(ghat){
    showRow('fetching_data')
    browser.runtime.sendMessage(browser.runtime.id,{command:'fetch_data_from_github'})
  }
...
window.onload = function(){
  showRow('fetching_data')
  browser.storage.sync.get(['ghUserData', 'ghat', 'chainLength'],
...
$i('connect_to_github_btn').addEventListener('click', function(){
  showRow('connecting')
  browser.runtime.sendMessage(browser.runtime.id,{command:'connect_to_github'})
})
