...
chrome.storage.sync.get('ghUserData',({ghUserData: userData}) => {
...
function disconnect(){
  chrome.storage.sync.remove(['ghUserData','ghat','chainLength'])
...
function save(event){
  ...
  if(isNumeric(refresh_rate)){
    chrome.storage.sync.set({'refreshRate':refresh_rate})
    window.close()
  }else{
